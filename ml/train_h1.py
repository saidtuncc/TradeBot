#!/usr/bin/env python
"""
Retrain H1 Strategic Model — New pipeline (V4) with Optuna + Calibration
Replaces legacy lgb_*.bin with h1_long.pkl / h1_short.pkl
Usage: python ml/train_h1.py
"""
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("H1 STRATEGIC MODEL TRAINING (V4 Pipeline)")
    logger.info("=" * 60)

    # 1. Build H1 feature set (existing pipeline, no news for speed)
    df = build_full_feature_set('.', add_news=False)
    ...
    # 3. Train LONG
    logger.info("\n" + "=" * 50)
    logger.info("TRAINING H1 LONG MODEL (V4)")
    logger.info("=" * 50)
    _train_and_save(train_df, test_df, 'target_long', 'h1_long')

    # 4. Train SHORT
    logger.info("\n" + "=" * 50)
    logger.info("TRAINING H1 SHORT MODEL (V4)")
    logger.info("=" * 50)
    _train_and_save(train_df, test_df, 'target_short', 'h1_short')

    logger.info("\n✅ H1 V4 model training complete!")
...
def _train_and_save(train_df, test_df, target_col, model_name):
    """Full pipeline: feature selection → Optuna → train → calibrate → evaluate."""
    ...
    # Step 1: Feature selection (top 40)
    logger.info("Step 1: Feature selection...")
    selected = _select_top_features(X_train, y_train, feature_cols, top_n=40)
    ...
    # Step 2: Optuna (200 trials, TimeSeriesCV)
    logger.info("Step 2: Optuna (200 trials, TimeSeriesCV 4-fold)...")
    best_params = _optuna_tune(X_train_sel, y_train, n_trials=200)
    logger.info("Best params: %s", best_params)

    # Step 3: Train final model with early stopping
    logger.info("Step 3: Training final model (500 rounds, early stop 50)...")
...
def _select_top_features(X, y, feature_cols, top_n=40):
...
def _optuna_tune(X, y, n_trials=200):
    """Optuna hyperparameter search with TimeSeriesCV."""
    import optuna
    import lightgbm as lgb
